    ...
    <div className="settings-page">
      ...
      <div className="settings-group">
        ...
        <div className="setting-row">
          <span className="setting-label"><User size={16} /> Username</span>
          <span className="setting-val">{user?.username || '—'}</span>
        </div>
        <div className="setting-row">
          <span className="setting-label"><Mail size={16} /> Email</span>
          <span className="setting-val">{user?.email || '—'}</span>
        </div>
        <div className="setting-row">
          <span className="setting-label"><Shield size={16} /> Account type</span>
      ...
      <div className="settings-group">
        ...
        <div className="setting-row">
          <span className="setting-label"><Music size={16} /> Instrument</span>
          <select aria-label="Default instrument" className="setting-select" value={user?.instrument_preference || ''} onChange={handleInstrumentChange} disabled={!user?.id}>
        ...
        <p className="setting-hint">{user?.id ? 'Used when a district supports both instruments.' : 'Sign in to save an instrument preference.'}</p>
        <div className="setting-row">
          <span className="setting-label"><Shield size={16} /> Practice level</span>
          <select aria-label="Practice level" className="setting-select" value={user?.skill_level || ''} onChange={handleSkillLevelChange} disabled={!user?.id}>
      ...
      <div className="settings-group">
        <h3>World display</h3>
        <div className="setting-row">
          <span className="setting-label"><Activity size={16} /> World motion</span>
          <select aria-label="World motion" className="setting-select" value={motion} onChange={(event) => setMotion(event.target.value as 'full' | 'comfort' | 'minimal')}>
            <option value="full">Full</option>
            <option value="comfort">Comfort</option>
            <option value="minimal">Minimal</option>
          </select>
        </div>
        <p className="setting-hint">Minimal removes camera travel and non-essential displacement. Device reduced-motion is respected automatically.</p>
        <div className="setting-row">
          <span className="setting-label"><Gauge size={16} /> Graphics profile</span>
          <select aria-label="Graphics profile" className="setting-select" value={performance} onChange={(event) => setPerformance(event.target.value as 'high' | 'balanced' | 'low')}>
      ...
      <div className="settings-group audio-mix-settings">
        <h3><Volume2 size={17} /> Sound mix</h3>
        <p className="setting-hint">Educational prompts stay independent from ambience, feedback, characters, rewards, and accessibility cues.</p>
        {AUDIO_CATEGORIES.map((category) => (
          <div className="audio-mix-row" key={category.id}>
            <label htmlFor={`audio-${category.id}`}><strong>{category.label}</strong><small>{category.description}</small></label>
            <input id={`audio-${category.id}`} type="range" min="0" max="100" step="5" value={Math.round(audioMix[category.id] * 100)} onChange={(event) => handleAudioMixChange(category.id, Number(event.target.value) / 100)} />
      ...
      <div className="settings-group">
        <h3>Privacy</h3>
        <div className="setting-row">
          <span className="setting-label"><Eye size={16} /> Improve learning activities</span>
          <label className="setting-switch">
            <input type="checkbox" checked={Boolean(user?.id && user?.analytics_enabled !== false)} disabled={!user?.id} onChange={handleAnalyticsChange} />
        ...
        {privacyStatus && <p className="setting-status" role="status">{privacyStatus}</p>}
      </div>
    </div>
  );
};
