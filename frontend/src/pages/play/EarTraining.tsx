...
const EarTraining = () => {
  ...
  const [audioState, setAudioState] = useState({ audioReady: false, loadedInstrumentIds: [], loadingInstrumentId: null });
  ...
  const updateControlMapping = useCallback((action, code) => {
  ...
  const playPrompt = useCallback(async ({ slow = false, replay = false, preservePhase = false } = {}) => {
    ...
    try {
      ...
      if (exercise.chords?.length > 1) {
        playback = await audioRef.current.playChordSequence({ instrumentId: selectedInstrument, chords: exercise.chords, timingScale });
      } else if (exercise.chords?.length === 1) {
        playback = await audioRef.current.playChord({ instrumentId: selectedInstrument, notes: exercise.chords[0], timingScale });
      } else if (exercise.notes?.length > 2 || ['shape', 'direction'].includes(exercise.type)) {
        playback = await audioRef.current.playNoteSequence({ instrumentId: selectedInstrument, notes: exercise.notes, timingScale });
      } else {
      ...
      playbackTimerRef.current = window.setTimeout(() => {
        ...
        setAnnouncement(`Prompt finished. ${challenge.answers[gameRef.current.avatarLane]?.label || 'First gate'} selected.`);
      }, playback.durationMs);
    } catch {
      dispatch({ type: 'ERROR', error: 'The musical prompt could not play. Check browser sound permissions and try again.' });
  ...
  const scheduleNext = useCallback((delay = 2600) => {
  ...
  const avatarState = game.phase === 'playing-prompt'
    ? 'listening'
  ...
  if (game.phase === 'loading' && !challenge) {
    return <main className="note-runner note-runner--loading"><Headphones /><h1>Note Runner</h1><p>Building the listening stage…</p></main>;
  }
  ...
  if (game.phase === 'error') {
    return (
      <main className="note-runner note-runner--error">
        ...
        <p>{game.error}</p>
        <button type="button" onClick={() => loadChallenge()}>Try again</button>
      </main>
    );
  }
  ...
  if (game.phase === 'run-complete') {
    ...
    const rankEvent = progressState.lastRankEvent?.runId === game.runId
      ? progressState.lastRankEvent
      : null;
    const rankMessage = rankEvent?.type === 'rank-up'
      ...
      {game.phase === 'paused' && !settingsOpen && (
        <div className="game-modal" role="dialog" aria-modal="true" aria-labelledby="pause-title" ref={modalRef}>
          <div className="game-modal__panel">
            ...
            <button type="button" className="note-runner__primary" autoFocus onClick={() => dispatchGameAction('pause')}>Resume run</button>
      ...
      {settingsOpen && (
        <div className="game-modal" role="dialog" aria-modal="true" aria-labelledby="settings-title" ref={modalRef}>
          <div className="game-modal__panel game-modal__panel--settings">
            ...
            <label><span>Reduced motion<small>Replace movement with state fades.</small></span><input type="checkbox" checked={settings.reducedMotion} onChange={(event) => updateSettings({ reducedMotion: event.target.checked })} /></label>
            <label><span>High contrast<small>Increase gate and focus contrast.</small></span><input type="checkbox" checked={settings.highContrast} onChange={(event) => updateSettings({ highContrast: event.target.checked })} /></label>
            <label><span>Auto-advance<small>Move on after feedback automatically.</small></span><input type="checkbox" checked={settings.autoAdvance} onChange={(event) => updateSettings({ autoAdvance: event.target.checked })} /></label>
            <label><span>Unlimited response time<small>Never use speed for the confidence bonus.</small></span><input type="checkbox" checked={settings.unlimitedTime} onChange={(event) => updateSettings({ unlimitedTime: event.target.checked })} /></label>
            ...
            <button type="button" className="note-runner__primary" autoFocus onClick={closeSettings}>Done</button>
          </div>
        </div>
      )}
    </>
