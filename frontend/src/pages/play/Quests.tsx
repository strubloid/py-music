  ...
  const handleClaim = async (quest) => {
    ...
    try {
      ...
      if (reward.alreadyClaimed) {
        ...
        setMessage(
          `Seals released: +${reward.xpAwarded} XP${reward.focusRestored ? ` and +${reward.focusRestored} Focus` : ''}.`,
        )
        showCityReward({
    ...
    return (
      <main className="quest-vaults quest-vaults--chamber">
        <header className="vault-chamber__header">
          <span>
            <Sparkles size={15} /> QUEST VAULTS
          </span>
          <h1>The Resonance Chamber</h1>
          <p>
            Three mechanisms hold the City's practice missions. Choose one vault door; completed energy flows toward its
            reward object.
          </p>
          <div className="vault-chamber__meters">
            <span>
              <Trophy /> {rankMeta.name} · Level {rankMeta.accountLevel}
            </span>
            <span>
              <BatteryCharging /> {progressState.focusPoints}/10 Focus
            </span>
          </div>
        </header>

        <section className="vault-chamber" aria-label="Quest vault chamber">
          <div className="vault-chamber__rings" aria-hidden="true">
            <i />
            <i />
            <i />
          </div>
          <div className="vault-keeper" aria-label="Vault Keeper watches the chamber">
            <span>⌁</span>
            <b>
              VAULT
              <br />
          ...
          <div className="vault-pip" aria-label="Pip waits at the centre of the chamber">
            <i>♪</i>
            <span>Pip</span>
          </div>
          <div className="vault-energy" aria-hidden="true">
            <i />
            <i />
            <i />
            <i />
          ...
          <div className="vault-door-row">
            {vaultSummaries.map((vault) => {
              ...
              return (
                ...
                >
                  <span className="vault-door__arch">
                    <vault.Icon />
                    <i style={{ '--vault-progress': `${percent * 3.6}deg` }} />
                    <b>{percent}%</b>
                  </span>
                  <strong>{vault.label}</strong>
                  <span>
                    {vault.ready
                      ? `${vault.ready} ready to claim`
                      : `${vault.resolved}/${vault.missions.length} resonating`}
                  </span>
                  <small>{vault.featured?.quest.title}</small>
                  <em>{vault.reset}</em>
                  <span className="vault-door__reward" aria-hidden="true">
                    {vault.object}
                  </span>
                </button>
              )
  ...
  return (
    <main className={`quest-vaults quest-vaults--interior quest-vaults--${selectedVault.id}`}>
      <header className="vault-interior__header">
        <button type="button" onClick={() => setOpenVault(null)}>
          <ArrowLeft /> Return to chamber
        </button>
        <div>
          <span>{selectedVault.reset}</span>
          <h1>{selectedVault.label}</h1>
          <p>
            {selectedVault.missions.length} mission objects · {selectedVault.reward.toLocaleString()} XP in the full
            vault
          </p>
        </div>
      ...
      <section className="mission-vault" aria-label={`${selectedVault.label} mission objects`}>
        ...
        {selectedVault.states.map(({ quest, current, periodKey, claimed, complete }, index) => {
          ...
          return (
            <article
              className={`mission-object mission-object--${objectType} ${canClaim ? 'is-ready' : ''} ${claimed ? 'is-claimed' : ''}`}
              key={quest.id}
            >
              <div className="mission-object__artifact" aria-hidden="true">
                <i />
                <span>{String(index + 1).padStart(2, '0')}</span>
              </div>
              <div className="mission-object__copy">
                <span className="mission-object__reward">
                  +{quest.xp} XP {quest.focus ? `· +${quest.focus} Focus` : ''}
                </span>
                <h2>{quest.title}</h2>
                <p>{quest.description}</p>
                <div className="mission-seals" aria-label={`${current} of ${quest.target} complete`}>
                  {Array.from({ length: sealCount }, (_, seal) => (
                    <i className={seal < litSeals ? 'is-lit' : ''} key={seal} />
                  ))}
                  <strong>
                ...
                <button
                  ...
                  aria-label={
                    canClaim
                      ? `Claim reward: ${quest.title}`
                      : claimed
                        ? `Claimed: ${quest.title}`
                        : `Launch mission: ${quest.title}`
                  }
                  onClick={() => (canClaim ? handleClaim(quest) : navigate(getMissionRoute(quest)))}
                >
                  {claimed ? (
                    <>
                      <Check /> Stamped complete
                    </>
                  ) : canClaim ? (
                    <>
                      <KeyRound /> Release seals
                    </>
                  ) : (
                    <>Hand mission to Pip</>
                  )}
