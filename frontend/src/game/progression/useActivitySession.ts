...
import { completeActivity, mutateFocus, recordAnalyticsEvent, startActivity, useSoundGatesPower } from '../../services/api';
...
export const useActivitySession = (activity: string, active: boolean) => {
  ...
  useEffect(() => {
    ...
    setStatus('starting');
    startActivity(activity, sessionKeyRef.current)
      .then(({ data }) => {
  ...
  const spendFocus = useCallback(async (amount: number, reason: string) => {
  ...
  const activateSoundPower = useCallback(async (powerId: string, challengeId: number | string) => {
    if (!isLoggedIn || !startedRef.current || completedRef.current) return null;
