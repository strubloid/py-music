...
const PixiBackground = () => {
  ...
  useEffect(() => {
    ...
    const mount = async () => {
      ...
      if (cancelled || !hostRef.current) { app.destroy(true); return; }
      ...
      cleanup = () => { app.ticker.remove(ticker); app.destroy(true, { children: true }); };
    };

    void mount();
    return () => { cancelled = true; cleanup(); if (hostRef.current) hostRef.current.replaceChildren(); };
  }, [motion, performance]);
