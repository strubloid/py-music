...
const PhaserHost = ({ destinations, activeDestination, initialDestination, onDestinationNear }: PhaserHostProps) => {
  ...
  useEffect(() => { onDestinationNearRef.current = onDestinationNear; }, [onDestinationNear]);
  ...
  useEffect(() => {
    ...
    const mount = async () => {
      ...
      class PracticeSquareScene extends Phaser.Scene {
        ...
        create() {
          ...
          destinations.forEach((destination) => {
            ...
            const tower = this.add.rectangle(x, height * 0.5, 88, 170, destination.color, 0.24)
              .setStrokeStyle(2, destination.color, 0.8);
            const portal = this.add.ellipse(x, height * 0.67, 56, 82, destination.color, 0.22)
          ...
          this.input.on('pointerdown', (pointer: import('phaser').Input.Pointer) => { this.targetX = pointer.x; });
        }
      ...
      game = new Phaser.Game({
        ...
        render: { antialias: performance !== 'low', powerPreference: performance === 'high' ? 'high-performance' : 'default' },
