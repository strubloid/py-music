...
const DistrictLoading = () => {
  ...
  const copy = DISTRICT_LOADING_COPY[location.pathname] || ['Pip is tuning the district…', 'Loading only the music and scenery this place needs.']
  return (
    <div className="district-loading" role="status" aria-live="polite">
      <span className="loading-notes" aria-hidden="true">♪ ♫ ♪</span>
  ...
  render() {
    ...
    return (
      <section className="district-error" role="alert">
        ...
        <div><button type="button" onClick={() => window.location.reload()}>Reconnect signal</button><a href="/">Return to Practice Square</a></div>
...
const App = () => {
  ...
  return (
    ...
    <div className={`app-shell ${isWorldRoute ? 'world-mode' : ''}`}>
      ...
      {/* Main workspace */}
      <main className={`workspace ${sidebarCollapsed ? 'sidebar-collapsed' : ''} ${isWorldRoute ? 'world-workspace' : ''}`}>
        <ChordPanelProvider>
