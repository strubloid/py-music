...
export const startActivity = (activity, sessionKey) => api.post("/api/game/activity-start", { activity, session_key: sessionKey });
...
export const recordAnalyticsEvent = (event, activity, properties = {}) => api.post("/api/analytics/events", { event, activity, properties });
...
export const completeDailyChallenge = (challengeId, payload = {}) => api.post(`/api/daily-challenge/${challengeId}/complete`, payload);
...
if (typeof window !== "undefined") {
    const send = (msg, source, lineno, colno, err) => {
        logError(err ? "FRONTEND" : "JS_ERROR", err?.message || msg, [`Source: ${source}`, `Line: ${lineno}  Col: ${colno}`, err ? `Stack: ${err.stack}` : ""].filter(Boolean).join("\n")).catch(() => {}); // fire-and-forget
    };
