...
const ScaleInfo = ({ scaleData }) => {
    ...
    return (
      <div className="change-chord-block">
        <div className="change-chord-header">
          ...
          <button className="close-variations-btn" onClick={() => setSelectedChord(null)} aria-label="Close">
            ×
          </button>
        </div>
        <div className="chord-variations-list">
          {variations.map((chordData, index) => {
            ...
            return (
              ...
              >
                <div className="variation-label">{chordData.position || `Shape ${index + 1}`}</div>
                <div className="mini-chord-preview">{renderMiniChordDiagram(chordData, isSelected)}</div>
              </div>
            )
          })}
        </div>
      ...
      <svg viewBox="0 0 80 100" className="mini-chord-svg">
        ...
        {chordData.frets.map((fret, stringIndex) => {
          ...
          if (fret === 'x' || fret === 'X') {
            ...
            return (
              <circle key={`fret-${stringIndex}`} cx={x} cy="15" r="3" fill="none" stroke="#4CAF50" strokeWidth="1.5" />
            )
          } else {
  ...
  return (
    <div className="scale-info-container">
      <h2 className="scale-title">{scale_name}</h2>
          ...
          <div className="roman-numerals">
            {scale_degrees.map((degree, index) => (
              ...
              >
                ...
                {showChords ? (
                  <ChordDiagram chord={degree.chord} size="large" refreshTrigger={refreshKey} />
                ) : (
