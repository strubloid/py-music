...
const ScaleInfo = ({ scaleData }) => {
    ...
    return (
        <div className="bg-black/20 backdrop-blur-sm rounded-xl p-6 mb-8">
            <h2 className="text-3xl font-bold text-center mb-4 text-yellow-400">{scale_name}</h2>
                    ...
                    <div className="space-y-2">
                        {scale_degrees.map((degree, index) => (
                            <div key={index} className="flex justify-between items-center bg-white/5 rounded-lg p-3">
                                ...
                                <div className="flex items-center gap-3">
                                    {showChords ? <ChordDiagram chord={degree.chord} size="small" /> : <span className="text-green-400 chord-text-display">{degree.chord}</span>}
