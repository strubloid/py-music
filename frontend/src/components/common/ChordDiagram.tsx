...
const ChordDiagram = ({ chord, size = 'medium', refreshTrigger = null }) => {
  ...
  console.log(
    `Chord: ${chord}, Has Multiple: ${hasMultipleVariations}, Count: ${variationCount}, Current Index: ${currentVariationIndex}`,
  )
  ...
  return (
    <div className="chord-diagram-wrapper">
      <div className="chord-diagram-container" title={chordData.position || chord}>
        <svg width={width + 20} height={height + 20} className="chord-diagram">
          ...
          {showFretNumber && (
            <text x="0" y="34" fontSize="14" fontWeight="bold" fill="#333">
              {startFret}
            </text>
          )}
          ...
          {chordData.frets.map((fretStr, stringIndex) => {
            ...
            if (fret === -1) {
              ...
              return (
                <g key={stringIndex}>
                  <circle cx={x} cy="22" r="5" fill="white" stroke="#4CAF50" strokeWidth="3" />
          ...
          {/* Chord name */}
          <text x={width / 2} y={height - 2} textAnchor="middle" fontSize="12" fontWeight="bold" fill="#333">
            {chord}
          </text>
        </svg>
      </div>
