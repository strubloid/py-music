          ...
          <div className="strings">
            {stringNames.map((stringName, stringIndex) => (
              <div key={stringIndex} className="string">
                ...
                <div className="frets">
                  <div className="nut">
                    {chordData.frets[stringIndex] === '0' && <div className="open-string">○</div>}
                    {chordData.frets[stringIndex] === 'x' && <div className="muted-string">×</div>}
                  </div>
                  {[1, 2, 3, 4].map((fret) => (
        ...
        <div className="piano-keys-container">
          <div className="piano-keys">
            {keyOrder.map((note, index) => {
              const isPressed =
                notes.includes(note) || notes.includes(note.replace('#', '♯')) || notes.includes(note.replace('#', 'b'))
