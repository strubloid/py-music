...
const Info = ({ title, icon, children, side = 'left', offset = 0, initialExpanded = false, className = '' }) => {
  ...
  return (
    ...
    >
      ...
      {isExpanded && (
        <div ref={panelRef} className="info-panel" style={dynamicPosition}>
          <div className="info-header">
            ...
            <div className="info-controls">
              ...
              <button className="close-button" onClick={() => setIsExpanded(false)} title="Collapse panel">
                {getExpandIcon()}
              </button>
            </div>
          </div>

          {isVisible && <div className="info-content">{children}</div>}
        </div>
      )}
    </div>
  )
