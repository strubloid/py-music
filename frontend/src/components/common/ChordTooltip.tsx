...
const ChordTooltip = ({
        ...
        <div className="mini-piano">
          <div className="white-keys">
            {whiteKeys.map((note, index) => (
              <div key={note} className={`white-key ${notes.includes(note) ? 'active' : ''}`}>
            ...
            <div key={stringIndex} className="guitar-string-row">
              ...
              {[0, 1, 2, 3, 4].map((fret) => (
                <div key={fret} className="fret-position">
                  {fret === 0 && chordData.frets[stringIndex] === '0' && <div className="open-string">o</div>}
                  {fret === 0 && chordData.frets[stringIndex] === 'x' && <div className="muted-string">×</div>}
  ...
  return (
    <span className={`chord-tooltip-trigger ${className}`} onClick={handleClick}>
      {children}
