...
const ScaleChallengeInstruments = ({ root, steps }: ScaleChallengeInstrumentsProps) => {
  ...
  const scaleData = useMemo(() => {
    ...
    const scaleNotes = steps.reduce<string[]>((notes, step) => {
  ...
  return (
    <section className="scale-challenge-preview" aria-label={`${root} scale shape preview`}>
      <header>
        <div><span>Scale shape</span><strong>{root} root highlighted in gold</strong></div>
        <div className="scale-challenge-preview__tabs" role="tablist" aria-label="Scale shape instrument">
          <button type="button" role="tab" aria-selected={instrument === 'piano'} className={instrument === 'piano' ? 'is-active' : ''} onClick={() => setInstrument('piano')}>Piano</button>
          <button type="button" role="tab" aria-selected={instrument === 'guitar'} className={instrument === 'guitar' ? 'is-active' : ''} onClick={() => setInstrument('guitar')}>Fretboard</button>
        </div>
      </header>
      {instrument === 'piano'
