...
export function generateTier1Fragment(
  ...
  const wrongOnSameString = positions.find(
    (p) =>
      ...
      p.fret !== gap.fret &&
      p.fret <= 12 &&
      Math.abs(p.fret - gap.fret) >= 2,
  );
  const wrong = wrongOnSameString ?? positions.find(
    (p) =>
      p.stringIndex === gap.stringIndex - 1 &&
      p !== gap,
  ) ?? null;

  const candidates: Array<ScalePathPosition & { isCorrect: boolean }> = [
    { ...gap, isCorrect: true },
  ];
  ...
  const direction: 'left' | 'up' =
    gap.string === anchor.string ? 'left' : 'up';
...
function generateFromAvailable(
  ...
  const wrong = positions.find(
    (p) => p !== gap && Math.abs(p.fret - gap.fret) > 1,
  );
