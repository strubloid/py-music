...
export const scaleLabReducer = (state: ScaleLabState, event: { type: string; [key: string]: unknown }): ScaleLabState => {
  switch (event.type) {
    case 'SET_ROOT':
      return { ...state, root: event.root as string, selectedPositions: [], candidates: [], verifiedResult: null, phase: 'idle', explanation: null };

    case 'SET_MODE':
      return { ...state, mode: event.mode as string, selectedPositions: [], candidates: [], verifiedResult: null, phase: 'idle', explanation: null };

    case 'SET_RANGE':
      return { ...state, rangeLevel: event.level as number, selectedPositions: [], candidates: [], verifiedResult: null, phase: 'idle', explanation: null };

    case 'TOGGLE_POSITION': {
      const pos = event.position as LabPosition;
      const exists = state.selectedPositions.find(
        (p) => p.string === pos.string && p.fret === pos.fret,
      );
      const next = exists
        ? state.selectedPositions.filter((p) => p !== exists)
        : [...state.selectedPositions, pos];
      return {
        ...state,
    ...
    case 'CLEAR_ALL':
      return { ...state, selectedPositions: [], candidates: [], verifiedResult: null, phase: 'idle', explanation: null, showAllMissing: false };
