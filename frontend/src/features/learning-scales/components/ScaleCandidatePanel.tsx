...
const ScaleCandidatePanel: React.FC<ScaleCandidatePanelProps> = ({ candidates, rootPitch, targetMode }) => {
  ...
  return (
    <div className="sc-panel">
      <h3 className="sc-panel__title">{targetMode ? 'Target Scale' : 'Compatible Scales'}</h3>
      <ul className="sc-panel__list" role="list">
        {candidates.map((c) => {
          ...
          const missing = c.missingPitchClasses
            ...
            .join(', ')
          return (
            <li key={`${c.modeKey}-${c.modeName}`} className={`sc-candidate ${isTarget ? 'sc-candidate--target' : ''}`}>
