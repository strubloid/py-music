  ...
  return (
    <section className={`sound-formula ${result?.confirmed ? 'is-confirmed' : ''}`} aria-label="Sound Formula analysis">
      <header className="sound-formula__header">
        ...
        <div>
          <span>Sound Formula</span>
          <strong>{result?.analysisEngine === 'music21' ? 'music21 laboratory analysis' : 'Awaiting analysis'}</strong>
        </div>
      </header>
      ...
      {result && (
        <>
          <div className="sound-formula__equation" aria-label={`Formula ${result.formulaText}`}>
            {result.formula.map((degree, index) => (
              <React.Fragment key={`${degree}-${index}`}>
                <b>{degree}</b>
                {index < result.formula.length - 1 && <i>+</i>}
              </React.Fragment>
            ))}
          </div>
          <p className="sound-formula__character">
            <Atom size={16} /> Characteristic degree: <strong>{result.characteristicDegree}</strong>
          </p>
          ...
          {(missingNotes.length > 0 || extraNotes.length > 0) && (
            <div className="sound-formula__corrections">
              {missingNotes.length > 0 && (
                <p>
                  <span>Add</span> {missingNotes.join(' · ')}
                </p>
              )}
              {extraNotes.length > 0 && (
                <p>
                  <span>Outside formula</span> {extraNotes.join(' · ')}
                </p>
              )}
            </div>
          )}
          ...
          <div className="sound-formula__intervals">
            ...
            <strong>
              {result.selectedIntervals.length
                ? result.selectedIntervals.join(' · ')
                : 'Place notes to measure intervals'}
            </strong>
          </div>

          <div className="sound-formula__candidates">
            <h3>
              <Sparkles size={15} /> Closest formulas
            </h3>
            {result.candidates.slice(0, 4).map((candidate) => (
              <article key={`${candidate.root}-${candidate.mode}`} className={candidate.confirmed ? 'is-exact' : ''}>
                <div>
                  ...
                  <span>{candidate.formulaText}</span>
                </div>
