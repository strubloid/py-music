...
const HologramLayer = ({ answers, result }) => (
  <div className="hologram-layer" aria-label="Comparison hologram deck" data-comparison-mode="hologram" role="status">
    <header>
      <span>HOLOGRAM DECK</span>
      <strong>A/B SIGNAL ANALYSIS</strong>
    </header>
    <div className="hologram-layer__cards">
      {answers.map((answer) => (
        <article
          key={answer.id}
          className={
            answer.label === result.selectedLabel
              ? 'is-selected'
              : answer.label === result.correctLabel
                ? 'is-correct'
                : ''
          }
        >
