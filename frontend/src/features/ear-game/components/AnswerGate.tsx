...
const AnswerGate = ({
  ...
  return (
    ...
    >
      ...
      <span className="game-gate__frame" aria-hidden="true">
        <span className="game-gate__card">
          <span className="game-gate__back">
            <i>♪</i>
            <em>Signal locked</em>
          </span>
          <span className="game-gate__front">
            <i>{presentation.icon}</i>
            <em>{presentation.object}</em>
            <b />
          </span>
        </span>
      </span>
      <span className="game-gate__lane">{answer.lane + 1}</span>
      <span className="game-gate__label">
        <strong>{revealed ? answer.label : 'Listening…'}</strong>
        <small>
          {hidden ? 'Eliminated' : selected ? 'Commit · Enter' : revealed ? presentation.action : 'Signal locked'}
        </small>
      </span>
