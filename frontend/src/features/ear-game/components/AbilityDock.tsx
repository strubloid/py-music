...
const iconFor = (id) =>
  id.includes('combo') || id === 'second_chance'
    ? Shield
    : id === 'slow_down'
      ? Gauge
      : id === 'compare_mode'
    ...
    <label className="ability-dock__instrument">
      ...
      <span>
        ...
        <select
          ...
          disabled={game.phase === 'playing-prompt'}
        >
      ...
      <em>{audioState.loadingInstrumentId ? 'Loading samples' : 'Balanced voicing'}</em>
    </label>
      ...
      {powers.map((power) => {
        ...
        return (
          ...
          >
            ...
            <small>
              {power.focusCost ? `${power.focusCost} focus` : power.xpPenalty ? `−${power.xpPenalty} XP` : 'aid'}
            </small>
          </button>
        )
      })}
    </div>
    <div className="focus-crystal">
      <span>Focus</span>
      <strong>{focus}</strong>
    </div>
  </section>
