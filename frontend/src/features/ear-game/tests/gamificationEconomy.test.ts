...
import { CHALLENGE_XP_MULTIPLIER, getModeBaseXp } from '../../../game/rewardSystem'
import { QUEST_CATALOG, getQuestPeriodKey, getQuestProgress, selectQuestBoard } from '../../../game/questSystem'
...
test('quest catalog has intentional daily, weekly, and lifetime reward pools', () => {
  ...
  assert.equal(
    QUEST_CATALOG.filter((quest) => quest.cadence === 'daily').reduce((sum, quest) => sum + quest.xp, 0),
    100,
  )
  assert.equal(
    QUEST_CATALOG.filter((quest) => quest.cadence === 'weekly').reduce((sum, quest) => sum + quest.xp, 0),
    700,
  )
  assert.equal(
    QUEST_CATALOG.filter((quest) => quest.cadence === 'milestone').reduce((sum, quest) => sum + quest.xp, 0),
    10000,
  )
...
test('quest board is stable for a date and mixes daily, weekly, and milestone goals', () => {
  ...
  assert.deepEqual(
    first.map((quest) => quest.id),
    second.map((quest) => quest.id),
  )
