...
export const earGameReducer = (state, event) => {
  switch (event.type) {
    ...
    case 'MOVE': {
      ...
      return answer ? { ...state, avatarLane: nextLane, selectedAnswerId: answer.id, inputMode: event.inputMode || 'keyboard' } : state;
    }
    case 'SELECT_ANSWER': {
      if (isGameInputLocked(state)) return state;
      const answer = state.challenge?.answers?.find((item) => item.id === event.answerId);
      return answer ? { ...state, avatarLane: answer.lane, selectedAnswerId: answer.id, inputMode: event.inputMode || state.inputMode } : state;
    }
    case 'COMMIT_ANSWER':
    ...
    case 'NEXT_ROUND': {
      ...
      return { ...state, phase: 'round-transition', challengeIndex: nextIndex, challenge: null, selectedAnswerId: null, inputLocked: true };
    }
    case 'REPLAY':
      return { ...state, replayCount: state.replayCount + 1 };
    case 'POWER_USED':
      return state.usedPowers.includes(event.powerId) ? state : { ...state, usedPowers: [...state.usedPowers, event.powerId] };
    case 'PAUSE':
