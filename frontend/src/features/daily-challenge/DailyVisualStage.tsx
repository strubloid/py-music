...
const IntervalJourney = ({ journey }) => {
  ...
  return (
    <svg
      className="daily-interval-journey"
...
const InstrumentFrame = ({ notes, root, label, mode, children = null, journey = null }) => {
  ...
  return (
    <div className="daily-instrument-frame" aria-label={label}>
      {mode === 'guitar' ? (
        <GuitarFretboard fretboardData={fretboardData} fretCount={12} />
      ) : (
        <div className={journey ? 'daily-interval-piano' : undefined}>
          <PianoKeyboard keyboardData={keyboardData} />
          {journey && <IntervalJourney journey={journey} />}
        </div>
      )}
      {children}
    </div>
...
const ScaleVisual = ({ visual, mode }) => (
  <div className="daily-visual-stage__scale">
    ...
    <InstrumentFrame
      notes={visual.notes}
      root={visual.root}
      label={`${visual.root} scale shape with ${visual.notes?.join(', ')}`}
      mode={mode}
    />
  ...
  <div className={`daily-visual-stage__chords ${visual.chords?.length > 1 ? 'is-comparison' : ''}`}>
    {(visual.chords || []).map((chord, index) => (
      <ChordPanel
        ...
        chord={chord}
        mode={mode}
        label={visual.chords.length > 1 ? String.fromCharCode(65 + index) : 'Chord tones'}
      />
    ))}
  </div>
)
...
const IntervalVisual = ({ visual, mode, playbackPhase }) => {
  ...
  const distance = Math.max(
    1,
    Math.min(11, Number(visual.semitones) || Math.abs(pitchClass(notes[1]) - pitchClass(notes[0])) || 1),
  )
  return (
    <div className="daily-visual-stage__interval" role="img" aria-label={`Interval from ${notes[0]} to ${notes[1]}`}>
      <div className="daily-interval-staff" aria-hidden="true">
        ...
        <b
          className="daily-interval-note daily-interval-note--end"
          style={{ '--interval-rise': `${Math.max(8, Math.min(62, distance * 5))}%` }}
        />
      ...
      <InstrumentFrame
        notes={notes}
        root={notes[0]}
        label={`Interval journey from ${notes[0]} to ${notes[1]}`}
        mode={mode}
        journey={{ from: notes[0], to: notes[1], direction: visual.direction, phase: playbackPhase }}
      />
    </div>
  )
}
...
const ConceptVisual = ({ visual }) => {
  if (visual.kind === 'harmony')
    return (
      <div
        className="daily-visual-stage__harmony"
        role="img"
        aria-label={`Harmony in ${visual.key || 'C'}: ${(visual.romanNumerals || []).join(' to ')}`}
      >
  ...
  if (visual.kind === 'tempo')
    return (
      <div
        className="daily-visual-stage__tempo"
        role="img"
        aria-label={`Tempo range ${visual.bpmRange?.join(' to ') || ''} beats per minute`}
      >
        <span>Slow</span>
        <i />
  ...
  if (visual.kind === 'fretboard')
    return (
      <div
        className="daily-visual-stage__strings"
        role="img"
        aria-label={`Guitar tuning: ${(visual.tuning || []).join(', ')}`}
      >
  ...
  if (visual.kind === 'melody')
    return (
      <div
        className="daily-visual-stage__melody"
        role="img"
        aria-label={`Three note melody: ${(visual.notes || []).join(', ')}`}
      >
  ...
  return (
    <div
      className="daily-visual-stage__concept"
      role="img"
      aria-label={`${visual.subject || visual.instrument || 'Music'} visual`}
    >
        ...
        exercise.chords?.length > 1
          ...
          : exercise.chords?.length === 1
            ...
            : exercise.notes?.length > 2
              ...
              : await engine.current.playInterval({
                  instrumentId: 'piano',
                  mode: 'melodic',
...
const legacyVisual = (category, question = '', title = '') => {
  ...
  if (category === 'scales') {
    ...
    return {
      ...
      notes: intervals.map((interval) => CHROMATIC[(pitchClass(root) + interval) % 12]),
    }
  }
  if (category === 'chords') {
        ...
        chords: [
          {
            ...
            intervals,
            degrees: ['1', '3', '5', '7'],
            notes: intervals.map((interval) => CHROMATIC[(pitchClass(match[1]) + interval) % 12]),
          },
        ],
      }
    }
  }
  if (/5th of C Major/i.test(question))
    return {
      kind: 'scale',
      root: 'C',
          ...
          <div className="daily-visual-stage__actions">
            {supportsInstrument && (
              <div className="daily-visual-stage__switch" role="group" aria-label="Visual instrument">
                ...
                <button
                  type="button"
                  className={instrument === 'off' ? 'is-active' : ''}
                  onClick={() => setInstrument('off')}
                >
                  Off
                </button>
              </div>
