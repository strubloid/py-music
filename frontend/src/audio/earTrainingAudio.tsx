  ...
  return {
    start: ({ note, time, duration = 0.5, velocity = 108 }) => {
      ...
      const level = Math.max(
        0.0001,
        Math.min(0.22, Math.max(0.04, Number(velocity) / 600)) * getAudioLevel('musicalPrompt'),
      )
...
export const createEarTrainingAudioEngine = ({ onStateChange }: { onStateChange?: (state: any) => void } = {}) => {
  let context = null
  let loader = null
    ...
    try {
      await Promise.race([
        ...
        new Promise((_, reject) =>
          window.setTimeout(() => reject(new Error('Sample loading timed out.')), SAMPLE_LOAD_TIMEOUT_MS),
        ),
      ])
