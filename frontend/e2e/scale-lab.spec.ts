...
test('Scale Lab confirms a complete Sound Formula through music21', async ({ page }) => {
  ...
  for (const note of ['C', 'D', 'E', 'F', 'G', 'A', 'B']) {
    await page.getByRole('button', { name: new RegExp(`, ${note}\\.`, 'i') }).first().click();
  }

  const analyzeResponse = page.waitForResponse((response) => response.url().includes('/api/scale-path/verify') && response.request().method() === 'POST');
